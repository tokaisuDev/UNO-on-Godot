...
func on_play_approval():
	prints("playing:", picked_card.card_color, picked_card.card_value)
	play_card_animation(picked_card)

func play_card(card):
	picked_card = card
	prints(picked_card.card_color, picked_card.card_value)
	$"../GameManager".request_play_card.rpc_id(1, picked_card.card_color, picked_card.card_value)

func play_card_as_opponent(card_color, card_value):
	if owned:
		return
